...
def create_server() -> FastMCP:
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "gms_history_values, gms_history_aggregate) over hand-written SQL — they encode the fixed "
            "PostgreSQL/Oracle join, zone, and batching logic. For long-range analysis (more than a "
            "day, or when only trends/statistics are needed) prefer gms_history_aggregate, which "
            "downsamples in Oracle, over gms_history_values' raw per-minute series. Fall back to "
            "db_query for ad-hoc or exploratory queries. "
