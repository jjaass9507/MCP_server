...
# on PATH.
#
# Adapted from ayghri/i-have-adhd's plugin hook: this skill is vendored into
# the project (not installed as a plugin), so the skill path is resolved via
# $CLAUDE_PROJECT_DIR instead of $CLAUDE_PLUGIN_ROOT.
...
skill_path="$project_dir/.claude/skills/i-have-adhd/SKILL.md"
[ -f "$skill_path" ] || exit 0
